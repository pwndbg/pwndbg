...
def get_file(path):
    ...
    """
    local_path = path
    qemu_root = pwndbglib.qemu.root()
    if qemu_root:
        return os.path.join(qemu_root, path)
    elif pwndbglib.remote.is_remote() and not pwndbglib.qemu.is_qemu():
        local_path = tempfile.mktemp(dir=pwndbglib.symbol.remote_files_dir)
        error      = None
        try:
...
def readlink(path):
    ...
    """
    is_qemu = pwndbglib.qemu.is_qemu_usermode()

    if is_qemu:
        if not os.path.exists(path):
            path = os.path.join(pwndbglib.qemu.root(), path)

    if is_qemu or not pwndbglib.remote.is_remote():
        try:

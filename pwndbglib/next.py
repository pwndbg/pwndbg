...
def next_int(address=None):
    ...
    """
    if address is None:
        ins = pwndbglib.disasm.one(pwndbglib.regs.pc)
        if not ins:
            return None
        address = ins.next

    ins = pwndbglib.disasm.one(address)
    while ins:
        if set(ins.groups) & jumps:
            return None
        if set(ins.groups) & interrupts:
            return ins
        ins = pwndbglib.disasm.one(ins.next)
...
def break_on_program_code():
    ...
    """
    mp = pwndbglib.proc.mem_page
    start = mp.start
    end = mp.end

    if start <= pwndbglib.regs.pc < end:
...
def break_on_next(address=None):
    address = address or pwndbglib.regs.pc
    ins = pwndbglib.disasm.one(address)

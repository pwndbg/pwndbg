...
from pwndbglib.color import strip
...
def get_config_parameters(scope, filter_pattern):
    values = [v for k, v in pwndbglib.config.__dict__.items()
              if isinstance(v, pwndbglib.config.Parameter) and v.scope == scope]
...
@pwndbglib.commands.ArgparsedCommand(configfile_parser)

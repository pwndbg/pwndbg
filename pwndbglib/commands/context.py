...
import pwndbglib.color.backtrace as B
...
import pwndbglib.commands.nearpc
...
from pwndbglib.color import theme
...
config_clear_screen = pwndbglib.config.Parameter('context-clear-screen', False, 'whether to clear the screen before printing the context')
config_output = pwndbglib.config.Parameter('context-output', 'stdout', 'where pwndbg should output ("stdout" or file/tty).')
config_context_sections = pwndbglib.config.Parameter('context-sections',
...
@pwndbglib.config.Trigger([config_context_sections])
...
@pwndbglib.commands.ArgparsedCommand(parser, aliases=['ctx-out'])
...
@pwndbglib.commands.ArgparsedCommand(parser, aliases=['ctx-watch', 'cwatch'])
...
@pwndbglib.commands.ArgparsedCommand(parser, aliases=['ctx-unwatch', 'cunwatch'])
...
def context_expressions(target=sys.stdout, with_banner=True, width=None):
    if not expressions:
        return []
    banner = [pwndbglib.ui.banner("expressions", target=target, width=width)]
    output = []
    if width is None:
        _height, width = pwndbglib.ui.get_window_size(target=target)
...
config_context_ghidra = pwndbglib.config.Parameter('context-ghidra',
                                                'never',
...
def context_ghidra(target=sys.stdout, with_banner=True, width=None):
    ...
    """
    banner = [pwndbglib.ui.banner("ghidra decompile", target=target, width=width)] if with_banner else []

    if config_context_ghidra == "never":
        return []

    if config_context_ghidra == "if-no-source":
        source_filename = pwndbglib.symbol.selected_frame_source_absolute_filename()
        if source_filename and os.path.exists(source_filename):
            return []

    try:
        return banner + pwndbglib.ghidra.decompile().split('\n')
...
@pwndbglib.commands.ArgparsedCommand(parser, aliases=['ctx'])
...
def context(subcontext=None):
    ...
    for target, res in result.items():
        ...
        if len(res) > 0 and settings.get("banner_bottom", True):
            with target as out:
                res.append(pwndbglib.ui.banner("", target=out,
                                               width=settings.get("width", None)))
...
pwndbglib.config.Parameter('show-compact-regs', False, 'whether to show a compact register view')
pwndbglib.config.Parameter('show-compact-regs-align', 20, 'the number of characters reserved for each register and value')
pwndbglib.config.Parameter('show-compact-regs-space', 4, 'the minimum number of characters separating each register')
pwndbglib.config.Parameter('show-compact-regs-columns', 0, 'the number of columns to show registers (if 0, using the show-compact-regs-align)')
...
def compact_regs(regs, width):
    align = int(pwndbglib.config.show_compact_regs_align)
    space = int(pwndbglib.config.show_compact_regs_space)
    columns = int(pwndbglib.config.show_compact_regs_columns)
    ...
    line_length = 0
    for reg in regs:
        reg_length = len(pwndbglib.color.strip(reg))
...
def context_regs(target=sys.stdout, with_banner=True, width=None):
    ...
    regs = get_regs()
    if pwndbglib.config.show_compact_regs:
        regs = compact_regs(regs, width)

    banner = [pwndbglib.ui.banner("registers", target=target, width=width)]
...
pwndbglib.config.Parameter('show-flags', False, 'whether to show flags registers')
pwndbglib.config.Parameter('show-retaddr-reg', False, 'whether to show return address register')


def get_regs(*regs):
    result = []

    if not regs and pwndbglib.config.show_retaddr_reg:
        regs = pwndbglib.regs.gpr + (pwndbglib.regs.frame, pwndbglib.regs.current.stack) + pwndbglib.regs.retaddr + (pwndbglib.regs.current.pc,)
    elif not regs:
        regs = pwndbglib.regs.gpr + (pwndbglib.regs.frame, pwndbglib.regs.current.stack, pwndbglib.regs.current.pc)

    if pwndbglib.config.show_flags:
        regs += tuple(pwndbglib.regs.flags)

    changed = pwndbglib.regs.changed

    for reg in regs:
        if reg is None:
            continue

        if reg not in pwndbglib.regs:
            message.warn("Unknown register: %r" % reg)
            continue

        value = pwndbglib.regs[reg]
        ...
        if reg in pwndbglib.regs.flags:
            desc = C.format_flags(value, pwndbglib.regs.flags[reg], pwndbglib.regs.last.get(reg, 0))

        else:
            desc = pwndbglib.chain.format(value)

        result.append("%s%s %s" % (m, regname, desc))

    return result

pwndbglib.config.Parameter('emulate', True, '''
Unicorn emulation of code near the current instruction
''')
code_lines = pwndbglib.config.Parameter('context-code-lines', 10, 'number of additional lines to print in the code context')
...
def context_disasm(target=sys.stdout, with_banner=True, width=None):
    ...
    syntax = pwndbglib.disasm.CapstoneSyntax[flavor]

    # Get the Capstone object to set disassembly syntax
    cs = next(iter(pwndbglib.disasm.get_disassembler_cached.cache.values()), None)
    ...
    banner = [pwndbglib.ui.banner("disasm", target=target, width=width)]
    emulate = bool(pwndbglib.config.emulate)
    result = pwndbglib.commands.nearpc.nearpc(to_string=True, emulate=emulate, lines=code_lines // 2)
...
source_code_lines = pwndbglib.config.Parameter('context-source-code-lines',
                                               10,
...
@pwndbglib.memoize.reset_on_start
...
def get_filename_and_formatted_source():
    ...
    prefix_sign = pwndbglib.config.code_prefix
    ...
    for line_number, code in enumerate(source, start=start + 1):
        ...
        if pwndbglib.config.highlight_source and line_number == closest_line:
            fmt = C.highlight(fmt)
...
def context_code(target=sys.stdout, with_banner=True, width=None):
    ...
    if formatted_source:
        bannerline = [pwndbglib.ui.banner("Source (code)", target=target, width=width)] if with_banner else []
        return bannerline + ['In file: %s' % filename] + formatted_source

    # Try getting source from IDA Pro Hex-Rays Decompiler
    if not pwndbglib.ida.available():
        return []
    ...
    code = pwndbglib.ida.decompile_context(pwndbglib.regs.pc, n)

    if code:
        bannerline = [pwndbglib.ui.banner("Hexrays pseudocode", target=target, width=width)] if with_banner else []
        return bannerline + code.splitlines()
    else:
        return []


stack_lines = pwndbglib.config.Parameter('context-stack-lines', 8, 'number of lines to print in the stack context')

def context_stack(target=sys.stdout, with_banner=True, width=None):
    result = [pwndbglib.ui.banner("stack", target=target, width=width)] if with_banner else []
    telescope = pwndbglib.commands.telescope.telescope(pwndbglib.regs.sp, to_string=True, count=stack_lines)
    if telescope:
        result.extend(telescope)
    return result


backtrace_lines = pwndbglib.config.Parameter('context-backtrace-lines', 8, 'number of lines to print in the backtrace context')
...
def context_backtrace(with_banner=True, target=sys.stdout, width=None):
    ...
    if with_banner:
        result.append(pwndbglib.ui.banner("backtrace", target=target, width=width))
    ...
    while True:
        ...
        addrsz = B.address(pwndbglib.ui.addrsz(frame.pc()))
        symbol = B.symbol(pwndbglib.symbol.get(frame.pc()))
        if symbol:
...
def context_args(with_banner=True, target=sys.stdout, width=None):
    args = pwndbglib.arguments.format_args(pwndbglib.disasm.one())
    ...
    if with_banner:
        args.insert(0, pwndbglib.ui.banner("arguments", target=target, width=width))
...
def save_signal(signal):
    ...
    if isinstance(signal, gdb.ExitedEvent):
        ...
        if signal.stop_signal == 'SIGSEGV':
            ...
            # see also issue 476
            if _is_rr_present():
                msg += ' (current pc: %#x)' % pwndbglib.regs.pc
            else:
                try:

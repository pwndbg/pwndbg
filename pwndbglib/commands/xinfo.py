...
def xinfo_stack(page, addr):
    ...
    frame = pwndbglib.regs[pwndbglib.regs.frame]
    frame_mapping = pwndbglib.vmmap.find(frame)
    ...
    canary_value = pwndbglib.commands.canary.canary_value()[0]

    if canary_value is not None:
        all_canaries = list(
            pwndbglib.search.search(pwndbglib.arch.pack(canary_value), mappings=pwndbglib.stack.stacks.values())
        )
...
def xinfo_mmap_file(page, addr):
    ...
    objpages = filter(lambda p: p.objfile == file_name, pwndbglib.vmmap.get())
    ...
    containing_loads = [seg for seg in pwndbglib.elf.get_containing_segments(file_name, first.vaddr, addr)
    ...
    containing_sections = pwndbglib.elf.get_containing_sections(file_name, first.vaddr, addr)
...
def xinfo(address=None):
    address = address.cast(pwndbglib.typeinfo.pvoid)  # Fixes issues with function ptrs (xinfo malloc)
    addr = int(address)
    addr &= pwndbglib.arch.ptrmask

    page = pwndbglib.vmmap.find(addr)

...
from pwndbglib.gdbutils.functions import GdbFunction


@pwndbglib.commands.ArgparsedCommand("Synchronize IDA's cursor with GDB")
@pwndbglib.commands.OnlyWhenRunning
@pwndbglib.events.stop
@pwndbglib.ida.withIDA
def j(*args):
    """
...
def up(n=1):
    ...
    bt = pwndbglib.commands.context.context_backtrace(with_banner=False)
    print('\n'.join(bt))
...
@pwndbglib.commands.ArgparsedCommand("Save the ida database.")
@pwndbglib.ida.withIDA
def save_ida():
    ...
    path = pwndbglib.ida.GetIdbPath()

    # Need to handle emulated paths for Wine
    if path.startswith('Z:'):
        path = path[2:].replace('\\', '/')
        pwndbglib.ida.SaveBase(path)
    ...
    pwndbglib.ida.SaveBase(full_path)
...
def ida(name):
    ...
    name = name.string()
    result = pwndbglib.ida.LocByName(name)

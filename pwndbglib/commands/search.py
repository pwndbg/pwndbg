...
def print_search_hit(address):
    ...
    vmmap = pwndbglib.vmmap.find(address)
    if vmmap:
    ...
    addr = M.get(address)
    display = pwndbglib.enhance.enhance(address)
    print(region, addr, display)

auto_save = pwndbglib.config.Parameter('auto-save-search', False,
...
def search(type, hex, string, executable, writable, value, mapping_name, save, next):
    ...
    if save is None:
        save = bool(pwndbglib.config.auto_save_search)
    ...
    if type not in ('string', 'bytes'):
        value = pwndbglib.commands.fix_int(value)
        value &= pwndbglib.arch.ptrmask
        fmt = {
            'little': '<',
            'big': '>'
    ...
    # Perform the search
    for address in pwndbglib.search.search(value,
                                           mappings=mappings,

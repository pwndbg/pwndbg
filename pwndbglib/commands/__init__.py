...
class Command(gdb.Command):
    ...
    def invoke(self, argument, from_tty):
        ...
        try:
            ...
            pwndbglib.exception.handle(self.function.__name__)
            return
...
def OnlyWhenHeapIsInitialized(function):
    ...
    def _OnlyWhenHeapIsInitialized(*a, **kw):
        if pwndbglib.heap.current.is_initialized():
            return function(*a, **kw)
        else:
...
def OnlyAmd64(function):
    ...
    @functools.wraps(function)
    def _OnlyAmd64(*a, **kw):
        if pwndbglib.arch.current == "x86-64":
            return function(*a, **kw)
        else:
...
def OnlyWithLibcDebugSyms(function):
    ...
    def _OnlyWithLibcDebugSyms(*a, **kw):
        if pwndbglib.heap.current.libc_has_debug_syms():
            return function(*a, **kw)
        else:

...
def format_bin(bins, verbose=False, offset=None):
    ...
    for size in bins:
        ...
        if bins_type == 'tcachebins':
            ...
            if count <= 7:
                limit = count + 1
            formatted_chain = pwndbglib.chain.format(chain_fd[0], offset=offset, limit=limit)
        else:
            formatted_chain = pwndbglib.chain.format(chain_fd[0], offset=offset)
        ...
        if is_chain_corrupted:
            ...
            line += message.hint('BK: ') + pwndbglib.chain.format(chain_bk[0], offset=allocator.chunk_key_offset('bk'))
        else:
...
@pwndbglib.commands.OnlyWithLibcDebugSyms
@pwndbglib.commands.OnlyWhenHeapIsInitialized
def heap(addr=None, verbose=False, simple=False):
    ...
    if addr:
        ...
        if pwndbglib.vmmap.find(allocator.get_heap(heap_region.start)['ar_ptr']) == heap_region:
    ...
    first_chunk_size = pwndbglib.arch.unpack(pwndbglib.memory.read(cursor + ptr_size, ptr_size))
    ...
    while cursor in heap_region:
        ...
        size_field = pwndbglib.memory.u(cursor + allocator.chunk_key_offset('size'))
...
@pwndbglib.commands.OnlyWithLibcDebugSyms
@pwndbglib.commands.OnlyWhenHeapIsInitialized
@pwndbglib.commands.OnlyWithTcache
...
def top_chunk(addr=None):
    ...
    arena = allocator.get_arena(addr)
    address = arena['top']
    size = pwndbglib.memory.u(int(address) + allocator.chunk_key_offset('size'))
...
def malloc_chunk(addr, fake=False, verbose=False, simple=False):
    ...
    for field_to_print in fields_ordered:
        if field_to_print in fields_to_print:
            out_fields += message.system(field_to_print) + ": 0x{:02x}\n".format(pwndbglib.memory.u(cursor + allocator.chunk_key_offset(field_to_print)))
...
def bins(addr=None, tcache_addr=None):
    ...
    """
    if pwndbglib.heap.current.has_tcache():
...
def find_fake_fast(addr, size=None):
    ...
    mem = pwndbglib.memory.read(start, max_fast - psize, partial=True)

    fmt = {
        'little': '<',
        'big': '>'
    }[pwndbglib.arch.endian] + {
        4: 'I',
        8: 'Q'
    }[psize]
...
def vis_heap_chunks(addr=None, count=None, naive=None):
    ...
    for _ in range(count + 1):
        ...
        size_field = pwndbglib.memory.u(cursor + ptr_size)
        real_size = size_field & ~allocator.malloc_align_mask
    ...
    for c, stop in enumerate(chunk_delims):
        ...
        while cursor != stop:
            ...
            cell = pwndbglib.arch.unpack(pwndbglib.memory.read(cursor, ptr_size))
            ...
            asc += bin_ascii(pwndbglib.memory.read(cursor, ptr_size))
            if printed % 2 == 0:
...
@pwndbglib.commands.ArgparsedCommand(try_free_parser)
@pwndbglib.commands.OnlyWhenRunning
@pwndbglib.commands.OnlyWhenHeapIsInitialized
def try_free(addr):
    addr = int(addr)
    ...
    # check hook
    free_hook = pwndbglib.symbol.address('__free_hook')
    if free_hook is not None:
        if pwndbglib.memory.pvoid(free_hook) != 0:
    ...
    ptr_size = pwndbglib.arch.ptrsize

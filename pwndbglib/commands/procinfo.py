...
class Process():
    def __init__(self, pid=None, tid=None):
        if pid is None:
            pid = pwndbglib.proc.pid
        if tid is None:
            tid = pwndbglib.proc.tid
        if not tid:
            tid = pid
        self.pid = pid
        self.tid = tid
    ...
    def selinux(self):
        ...
        raw = pwndbglib.file.get(path)
        return raw.decode().rstrip('\x00').strip()

    @property
    @pwndbglib.memoize.reset_on_stop
    def status(self):
        raw = pwndbglib.file.get('/proc/%i/task/%i/status' % (self.pid, self.tid))
    ...
    def open_files(self):
        ...
        for i in range(self.fdsize):
            link = pwndbglib.file.readlink('/proc/%i/fd/%i' % (pwndbglib.proc.pid, i))
    ...
    def connections(self):
        ...
        functions = [pwndbglib.net.tcp,
                     pwndbglib.net.unix,
                     pwndbglib.net.netlink]
...
@pwndbglib.commands.ArgparsedCommand("Gets the pid.")
@pwndbglib.commands.OnlyWhenRunning
def pid():
    print(pwndbglib.proc.pid)

@pwndbglib.commands.ArgparsedCommand("Display information about the running process.")
@pwndbglib.commands.OnlyWhenRunning
def procinfo():
    """
    ...
    exe  = str(pwndbglib.auxv.get()['AT_EXECFN'])
    print("%-10s %r" % ("exe", exe))
    ...
    if not pwndbglib.android.is_android():
        print("%-10s %s" % ("uid",     proc.uid))
        print("%-10s %s" % ("gid",     proc.gid))
        print("%-10s %s" % ("groups",  proc.groups))
    else:
        print("%-10s %s" % ("uid",     list(map(pwndbglib.android.aid_name, proc.uid))))
        print("%-10s %s" % ("gid",     list(map(pwndbglib.android.aid_name, proc.gid))))
        print("%-10s %s" % ("groups",  list(map(pwndbglib.android.aid_name, proc.groups))))

...
parser.add_argument('gdbval_or_str', type=pwndbglib.commands.sloppy_gdb_parse, nargs='?', default=None,
...
def vmmap_add(start, size, flags, offset):
    page_flags = {
        'r': pwndbglib.elf.PF_R,
        'w': pwndbglib.elf.PF_W,
        'x': pwndbglib.elf.PF_X,
    }
    perm = 0
    for flag in flags:
    ...
    page = pwndbglib.memory.Page(start, size, perm, offset)
    pwndbglib.vmmap.add_custom_page(page)

    print('%r added' % page)


@pwndbglib.commands.ArgparsedCommand("Clear the vmmap cache.") #TODO is this accurate?
def vmmap_clear():
    pwndbglib.vmmap.clear_custom_page()
...
def vmmap_load(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        for section in elffile.iter_sections():
            ...
            flags = pwndbglib.elf.PF_R
            if sh_flags & SH_FLAGS.SHF_WRITE:
                flags |= pwndbglib.elf.PF_W
            if sh_flags & SH_FLAGS.SHF_EXECINSTR:
                flags |= pwndbglib.elf.PF_X

            page = pwndbglib.memory.Page(vaddr, memsz, flags, offset, filename)
            pages.append(page)

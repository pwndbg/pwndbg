...
class Emulator:
    def __init__(self):
        self.arch = pwndbglib.arch.current
        ...
        self.regs = pwndbglib.regs.current
        ...
        for reg in list(self.regs.retaddr) + list(self.regs.misc) + list(self.regs.common) + list(self.regs.flags):
            ...
            if reg in blacklisted_regs:
                ...
                continue
            value = getattr(pwndbglib.regs, reg)
        ...
        self.map_page(pwndbglib.regs.pc)
    ...
    def get_uc_mode(self):
        ...
        """
        arch = pwndbglib.arch.current
        mode = 0

        if arch == 'armcm':
            mode |= (U.UC_MODE_MCLASS | U.UC_MODE_THUMB) if (pwndbglib.regs.xpsr & (1 << 24)) else U.UC_MODE_MCLASS

        elif arch in ('arm', 'aarch64'):
            mode |= U.UC_MODE_THUMB if (pwndbglib.regs.cpsr & (1 << 5)) else U.UC_MODE_ARM

        elif arch == 'mips' and 'isa32r6' in gdb.newest_frame().architecture().name():
            mode |= U.UC_MODE_MIPS32R6

        else:
            mode |= {4:U.UC_MODE_32, 8:U.UC_MODE_64}[pwndbglib.arch.ptrsize]
    ...
    def map_page(self, page):
        page = pwndbglib.memory.page_align(page)
        size = pwndbglib.memory.PAGE_SIZE

        debug("# Mapping %#x-%#x" % (page, page+size))

        try:
            data = pwndbglib.memory.read(page, size)
            data = bytes(data)
    ...
    def hook_mem_invalid(self, uc, access, address, size, value, user_data):
        ...
        start = pwndbglib.memory.page_align(address)
        size  = pwndbglib.memory.page_size_align(address + size - start)
        stop  = start + size

        # Map each page with the permissions that we think it has.
        for page in range(start, stop, pwndbglib.memory.PAGE_SIZE):
    ...
    def until_call(self, pc=None):
        ...
        while target and C.CS_GRP_CALL not in pwndbglib.disasm.one(addr).groups:

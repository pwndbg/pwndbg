...
ida_rpc_host = pwndbglib.config.Parameter('ida-rpc-host', '127.0.0.1', 'ida xmlrpc server address')
ida_rpc_port = pwndbglib.config.Parameter('ida-rpc-port', 31337, 'ida xmlrpc server port')
ida_enabled = pwndbglib.config.Parameter('ida-enabled', True, 'whether to enable ida integration')
ida_timeout = pwndbglib.config.Parameter('ida-timeout', 2, 'time to wait for ida xmlrpc in seconds')
...
@pwndbglib.decorators.only_after_first_prompt()
@pwndbglib.config.Trigger([ida_rpc_host, ida_rpc_port, ida_enabled, ida_timeout])
def init_ida_rpc_client():
    ...
    if exception:
        if not isinstance(_ida_last_exception, exception[0]) or _ida_last_exception.args != exception[1].args:
            if hasattr(pwndbglib.config, "exception_verbose") and pwndbglib.config.exception_verbose:
...
def l2r(addr):
    exe = pwndbglib.elf.exe()
    if not exe:
        raise Exception("Can't find EXE base")
    result = (addr - int(exe.address) + base()) & pwndbglib.arch.ptrmask
    return result
...
def r2l(addr):
    ...
    if not exe:
        raise Exception("Can't find EXE base")
    result = (addr - base() + int(exe.address)) & pwndbglib.arch.ptrmask
    return result
...
def UpdateBreakpoints():
    ...
    for bp in want - current:
        if not pwndbglib.memory.peek(bp):
            continue

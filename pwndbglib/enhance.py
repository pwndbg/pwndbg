...
import pwndbglib.color as color
import pwndbglib.color.enhance as E
...
from pwndbglib.color.syntax_highlight import syntax_highlight
...
def int_str(value):
    retval = '%#x' % int(value & pwndbglib.arch.ptrmask)

    # Try to unpack the value as a string
    packed = pwndbglib.arch.pack(int(value))
...
def enhance(value, code = True):
    ...
    name = pwndbglib.symbol.get(value) or None
    page = pwndbglib.vmmap.find(value)

    # If it's not in a page we know about, try to dereference
    # it anyway just to test.
    can_read = True
    if not page or None == pwndbglib.memory.peek(value):
        can_read = False
    ...
    if pwndbglib.ida.available() and not pwndbglib.ida.GetFunctionName(value):
        rwx = exe = False

    if exe:
        instr = pwndbglib.disasm.one(value)
        if instr:
            instr = "%-6s %s" % (instr.mnemonic, instr.op_str)
            if pwndbglib.config.syntax_highlight:
                instr = syntax_highlight(instr)

    szval = pwndbglib.strings.get(value) or None
    szval0 = szval
    if szval:
        szval = E.string(repr(szval))

    intval  = int(pwndbglib.memory.poi(pwndbglib.typeinfo.pvoid, value))
    intval0 = intval
    if 0 <= intval < 10:
        intval = E.integer(str(intval))
    else:
        intval = E.integer('%#x' % int(intval & pwndbglib.arch.ptrmask))
    ...
    # Otherwise strings have preference
        ...
        if len(szval0) < pwndbglib.arch.ptrsize:
            retval = [intval, szval]
        else:
            retval = [szval]

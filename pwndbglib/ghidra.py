...
def decompile(func=None):
    ...
    if not func:
        func = hex(pwndbglib.regs[pwndbglib.regs.current.pc]) if pwndbglib.proc.alive else 'main'
    ...
    if pwndbglib.proc.alive:
        pc = pwndbglib.regs[pwndbglib.regs.current.pc]
        ...
        if pos_annotations:
            ...
            line = source[curline]
            if line.startswith('    '):
                line = line[min(4, len(pwndbglib.config.code_prefix) + 1):]
    ...
    if pwndbglib.config.syntax_highlight:
        ...
        src_filename = pwndbglib.symbol.selected_frame_source_absolute_filename()
        if not src_filename:
    ...
    source = source.replace(current_line_marker, C.prefix(pwndbglib.config.code_prefix), 1)
    return source

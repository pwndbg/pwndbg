...
from pwndbglib.constants import ptmalloc
from pwndbglib.heap import heap_chain_limit
...
HEAP_MAX_SIZE = 1024 * 1024 if pwndbglib.arch.ptrsize == 4 else 2 * 4 * 1024 * 1024 * 8
...
class Arena:
    ...
    def __str__(self):
        res = []
        prefix = '[%%%ds]    ' % (pwndbglib.arch.ptrsize * 2)
        prefix_len = len(prefix % (''))
        arena_name = hex(self.addr) if self.addr != pwndbglib.heap.current.main_arena.address else 'main'
...
class HeapInfo:
    ...
    def __str__(self):
        fmt = '[%%%ds]' % (pwndbglib.arch.ptrsize * 2)
        return message.hint(fmt % (hex(self.first_chunk))) + M.heap(str(pwndbglib.vmmap.find(self.addr)))


class Heap(pwndbglib.heap.heap.BaseHeap):
    def __init__(self):
    ...
    @property
    def main_arena(self):
        main_arena_addr = pwndbglib.symbol.address('main_arena')

        if main_arena_addr is not None:
            self._main_arena = pwndbglib.memory.poi(self.malloc_state, main_arena_addr)
        else:
    ...
    def thread_cache(self):
        ...
        if self.has_tcache():
            ...
            if self.multithreaded:
                tcache_addr = pwndbglib.memory.pvoid(pwndbglib.symbol.address('tcache'))
                if tcache_addr != 0:
                    tcache = tcache_addr

            try:
                self._thread_cache = pwndbglib.memory.poi(self.tcache_perthread_struct, tcache)
    ...
    @property
    def mp(self):
        mp_addr = pwndbglib.symbol.address('mp_')

        if mp_addr is not None:
            self._mp = pwndbglib.memory.poi(self.malloc_par, mp_addr)

        return self._mp

    @property
    def global_max_fast(self):
        addr = pwndbglib.symbol.address('global_max_fast')
        return pwndbglib.memory.u(addr)

    @property
    @pwndbglib.memoize.reset_on_objfile
    def heap_info(self):
        return pwndbglib.typeinfo.load('heap_info')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def malloc_chunk(self):
        return pwndbglib.typeinfo.load('struct malloc_chunk')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def malloc_state(self):
        return pwndbglib.typeinfo.load('struct malloc_state')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def tcache_perthread_struct(self):
        return pwndbglib.typeinfo.load('struct tcache_perthread_struct')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def tcache_entry(self):
        return pwndbglib.typeinfo.load('struct tcache_entry')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def mallinfo(self):
        return pwndbglib.typeinfo.load('struct mallinfo')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def malloc_par(self):
        return pwndbglib.typeinfo.load('struct malloc_par')

    @property
    @pwndbglib.memoize.reset_on_objfile
    def malloc_alignment(self):
        """Corresponds to MALLOC_ALIGNMENT in glibc malloc.c"""
        return pwndbglib.arch.ptrsize * 2

    @property
    @pwndbglib.memoize.reset_on_objfile
    def size_sz(self):
        """Corresponds to SIZE_SZ in glibc malloc.c"""
        return pwndbglib.arch.ptrsize
    ...
    def min_chunk_size(self):
        ...
        return pwndbglib.arch.ptrsize * 4

    @property
    @pwndbglib.memoize.reset_on_objfile
    def multithreaded(self):
        """Is malloc operating within a multithreaded environment."""
        addr = pwndbglib.symbol.address('__libc_multiple_threads')
        if addr:
            return pwndbglib.memory.s32(addr) > 0
    ...
    def _spaces_table(self):
        spaces_table = [pwndbglib.arch.ptrsize * 2] * 64 \
                       + [ pow(2, 6) ] * 32 \
                       + [ pow(2, 9) ] * 16 \
                       + [ pow(2, 12) ] * 8 \
                       + [ pow(2, 15) ] * 4 \
    ...
    def chunk_key_offset(self, key):
        ...
        try:
            return chunk_keys.index(key) * pwndbglib.arch.ptrsize
        except:
            return None
    ...
    def tcache_next_offset(self):
        return self.tcache_entry.keys().index('next') * pwndbglib.arch.ptrsize

    def get_heap(self, addr):
        """Find & read the heap_info struct belonging to the chunk at 'addr'."""
        return pwndbglib.memory.poi(self.heap_info, heap_for_ptr(addr))
    ...
    def get_arena(self, arena_addr=None):
        ...
        if arena_addr is None:
            if self.multithreaded:
                arena_addr = pwndbglib.memory.u(pwndbglib.symbol.address('thread_arena'))
                if arena_addr != 0:
                    return pwndbglib.memory.poi(self.malloc_state, arena_addr)

            return self.main_arena

        try:
            next(i for i in pwndbglib.vmmap.get() if arena_addr in i)
            return pwndbglib.memory.poi(self.malloc_state, arena_addr)
    ...
    def get_arena_for_chunk(self, addr):
        chunk = pwndbglib.commands.heap.read_chunk(addr)
    ...
    def get_tcache(self, tcache_addr=None):
        ...
        return pwndbglib.memory.poi(self.tcache_perthread_struct, tcache_addr)
    ...
    def get_heap_boundaries(self, addr=None):
        ...
        page = pwndbglib.memory.Page(0, 0, 0, 0)
    ...
    def get_region(self, addr):
        ...
        return pwndbglib.vmmap.find(addr)
    ...
    def fastbins(self, arena_addr=None):
        ...
        num_fastbins = 7
        size         = pwndbglib.arch.ptrsize * 2

        result = OrderedDict()
        for i in range(num_fastbins):
            size += pwndbglib.arch.ptrsize * 2
            chain = pwndbglib.chain.get(int(fastbinsY[i]), offset=fd_offset, limit=heap_chain_limit)
    ...
    def tcachebins(self, tcache_addr=None):
        ...
        for i in range(num_tcachebins):
            ...
            count = int(counts[i])
            chain = pwndbglib.chain.get(int(entries[i]), offset=self.tcache_next_offset, limit=heap_chain_limit)
    ...
    def bin_at(self, index, arena_addr=None):
        ...
        bins_base    = int(normal_bins.address) - (pwndbglib.arch.ptrsize * 2)
        current_base = bins_base + (index * pwndbglib.arch.ptrsize * 2)
        ...
        get_chain = lambda bin, offset: pwndbglib.chain.get(int(bin), offset=offset, hard_stop=current_base, limit=heap_chain_limit, include_start=True)
    ...
    def largebin_index(self, sz):
        ...
        return self.largebin_index_64(sz) if pwndbglib.arch.ptrsize == 8 else self.largebin_index_32(sz)

    def is_initialized(self):
        addr = pwndbglib.symbol.address('__libc_malloc_initialized')
        return pwndbglib.memory.s32(addr) > 0

    def libc_has_debug_syms(self):
        return pwndbglib.symbol.address('global_max_fast') is not None

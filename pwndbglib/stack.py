...
def find_upper_stack_boundary(addr, max_pages=1024):
    ...
    if not pwndbglib.abi.linux:
        return addr + pwndbglib.memory.PAGE_SIZE

    return pwndbglib.elf.find_elf_magic(addr, max_pages=max_pages, ret_addr_anyway=True)
    ...
    try:
        for thread in gdb.selected_inferior().threads():
            ...
            if page is None:
                ...
                page  = pwndbglib.memory.Page(start, stop - start, 6 if not is_executable() else 7, 0, '[stack]')
...
def is_executable():
    ...
    ehdr         = pwndbglib.elf.exe()

    for phdr in pwndbglib.elf.iter_phdrs(ehdr):

...
def autofetch():
    ...
    """
    global remote_files_dir
    if not pwndbglib.remote.is_remote():
        return

    if pwndbglib.qemu.is_qemu_usermode():
        return

    if pwndbglib.android.is_android():
        return
    ...
    for mapping in pwndbglib.vmmap.get():
        ...
        try:
            data = pwndbglib.file.get(objfile)
            print('\r' + msg + ': OK')
        except OSError:
...
def get(address, gdb_only=False):
    ...
    """
    # Fast path
    if address < pwndbglib.memory.MMAP_MIN_ADDR or address >= ((1 << 64) - 1):
        return ''

    # Don't look up stack addresses
    if pwndbglib.stack.find(address):
        return ''
    ...
    if not gdb_only and result.startswith('No symbol'):
        ...
        if exe:
            exe_map = pwndbglib.vmmap.find(exe.address)
            if exe_map and address in exe_map:
                res = pwndbglib.ida.Name(address) or pwndbglib.ida.GetFuncOffset(address)
                return res or ''
...
def address(symbol, allow_unmapped=False):
    ...
    try:
        ...
        if not allow_unmapped and not pwndbglib.vmmap.find(address):
            return None
    ...
    try:
        address = pwndbglib.ida.LocByName(symbol)
        if address:
            return address
    except Exception:
        pass
...
def add_main_exe_to_symbols():
    ...
    mmap = pwndbglib.vmmap.find(addr)
    if not mmap:
        return

    path = mmap.objfile
    if path and (pwndbglib.arch.endian == pwndbglib.arch.native_endian):
        try:

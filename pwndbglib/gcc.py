...
def flags():
    ...
    if pwndbglib.arch.current == 'i386':
        return ['-m32']
    if pwndbglib.arch.current.endswith('x86-64'):
        return ['-m64']
...
def which():
    ...
    if not gcc:
        ...
        if pwndbglib.arch.ptrsize == 32:
            return ['g++','-m32']
        elif pwndbglib.arch.ptrsize == 64:
            return ['g++','-m32']

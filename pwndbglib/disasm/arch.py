...
class DisassemblyAssistant:
    ...
    def enhance(instruction):
        enhancer = DisassemblyAssistant.assistants.get(pwndbglib.arch.current, generic_assistant)
    ...
    def enhance_next(self, instruction):
        ...
        instruction.next = next_addr & pwndbglib.arch.ptrmask
        ...
        if op.type == CS_OP_MEM:
            ...
            if addr is not None:
                try:
                    ...
                    addr = int(pwndbglib.memory.poi(pwndbglib.typeinfo.ppvoid, addr))
    ...
    def enhance_operands(self, instruction):
        ...
        for i, op in enumerate(instruction.operands):
            ...
            if op.int:
                op.int &= pwndbglib.arch.ptrmask
            op.str = self.op_names.get(op.type, lambda *a: None)(instruction, op)

            if op.int:
                op.symbol = pwndbglib.symbol.get(op.int)

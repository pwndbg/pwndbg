...
class DisassemblyAssistant(pwndbglib.disasm.arch.DisassemblyAssistant):
    def regs(self, instruction, reg):
        if reg == X86_REG_RIP:
            return instruction.address + instruction.size
        elif instruction.address == pwndbglib.regs.pc:
            name = instruction.reg_name(reg)
            return pwndbglib.regs[name]
        else:
            return None

    def memory(self, instruction, op):
        current = (instruction.address == pwndbglib.regs.pc)
    ...
    def memory_sz(self, instruction, op):
        ...
        if index != 0:
            ...
            index = pwndbglib.regs[instruction.reg_name(index)]
            sz += "%s*%#x" % (index, scale)
            arith = True
    ...
    def next(self, instruction, call=False):
        ...
        address = (pwndbglib.regs.sp) + (pwndbglib.arch.ptrsize * pop)

        if pwndbglib.memory.peek(address):
            return int(pwndbglib.memory.poi(pwndbglib.typeinfo.ppvoid, address))

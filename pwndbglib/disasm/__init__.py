...
def get_disassembler(pc):
    if pwndbglib.arch.current == 'armcm':
        extra = (CS_MODE_MCLASS | CS_MODE_THUMB) if (pwndbglib.regs.xpsr & (1 << 24)) else CS_MODE_MCLASS

    elif pwndbglib.arch.current in ('arm', 'aarch64'):
        extra = CS_MODE_THUMB if (pwndbglib.regs.cpsr & (1 << 5)) else CS_MODE_ARM

    elif pwndbglib.arch.current == 'sparc':
    ...
    elif pwndbglib.arch.current == 'i8086':
        extra = CS_MODE_16

    elif pwndbglib.arch.current == 'mips' and 'isa32r6' in gdb.newest_frame().architecture().name():
        extra = CS_MODE_MIPS32R6
    
    else:
        extra = None

    return get_disassembler_cached(pwndbglib.arch.current,
                                   pwndbglib.arch.ptrsize,
                                   pwndbglib.arch.endian,
                                   extra)

@pwndbglib.memoize.reset_on_cont
def get_one_instruction(address):
    md   = get_disassembler(address)
    size = VariableInstructionSizeMax.get(pwndbglib.arch.current, 4)
    data = pwndbglib.memory.read(address, size, partial=True)
    for ins in md.disasm(bytes(data), address, 1):
        pwndbglib.disasm.arch.DisassemblyAssistant.enhance(ins)
        return ins

def one(address=None):
    if address is None:
        address = pwndbglib.regs.pc
    if not pwndbglib.memory.peek(address):
        return None
...
def near(address, instructions=1, emulate=False, show_prev_insns=True):
    ...
    if current is None or not pwndbglib.memory.peek(address):
        return []
    ...
    if not pwndbglib.emu or pwndbglib.arch.current not in pwndbglib.emu.emulator.arch_to_UC:
        emulate = False
    ...
    if address == pc and emulate:
        emu = pwndbglib.emu.emulator.Emulator()
        # skip current line
        emu.single_step()

...
class module(ModuleType):
    ...
    @pwndbglib.memoize.reset_on_prompt
    def __getattr__(self, attr):
        ...
        try:
            ...
            if 'eflags' in attr or 'cpsr' in attr:
                ...
                value = value.cast(pwndbglib.typeinfo.uint32)
            else:
                if attr.lower() == 'xpsr':
                    attr = 'xPSR'
                value = get_register(attr)
                size = pwndbglib.typeinfo.unsigned.get(value.type.sizeof, pwndbglib.typeinfo.ulong)
                value = value.cast(size)
                if attr.lower() == 'pc' and pwndbglib.arch.current == 'i8086':
                    value += self.cs * 16

            value = int(value)
            return value & pwndbglib.arch.ptrmask
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int):
            return int(item) & pwndbglib.arch.ptrmask

        return item

    def __iter__(self):
        regs = set(arch_to_regs[pwndbglib.arch.current]) | {'pc', 'sp'}
        for item in regs:
            yield item

    @property
    def current(self):
        return arch_to_regs[pwndbglib.arch.current]

    @property
    def gpr(self):
        return arch_to_regs[pwndbglib.arch.current].gpr

    @property
    def common(self):
        return arch_to_regs[pwndbglib.arch.current].common

    @property
    def frame(self):
        return arch_to_regs[pwndbglib.arch.current].frame

    @property
    def retaddr(self):
        return arch_to_regs[pwndbglib.arch.current].retaddr

    @property
    def flags(self):
        return arch_to_regs[pwndbglib.arch.current].flags

    @property
    def stack(self):
        return arch_to_regs[pwndbglib.arch.current].stack

    @property
    def retval(self):
        return arch_to_regs[pwndbglib.arch.current].retval

    @property
    def all(self):
        regs = arch_to_regs[pwndbglib.arch.current]
        retval = []
    ...
    def _fs_gs_helper(self, which):
        ...
        if pwndbglib.remote.is_remote(): return 0
        ...
        if result == 0:
            return (value.contents.value or 0) & pwndbglib.arch.ptrmask
...
def update_last():
    ...
    if pwndbglib.config.show_retaddr_reg:

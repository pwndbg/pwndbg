...
import pwndbglib.elftypes
import pwndbglib.events
import pwndbglib.info
...
def update():
    reload_module(pwndbglib.elftypes)

    if pwndbglib.arch.ptrsize == 4:
        Ehdr = pwndbglib.elftypes.Elf32_Ehdr
        Phdr = pwndbglib.elftypes.Elf32_Phdr
    else:
        Ehdr = pwndbglib.elftypes.Elf64_Ehdr
        Phdr = pwndbglib.elftypes.Elf64_Phdr
...
def read(typ, address, blob=None):
    ...
    if not blob:
        data = pwndbglib.memory.read(address, size)
    else:
...
def get_elf_info(filepath):
    ...
    """
    local_path = pwndbglib.file.get_file(filepath)
...
@pwndbglib.proc.OnlyWhenRunning
...
def entry():
    ...
    """
    entry = pwndbglib.auxv.get().AT_ENTRY
    if entry:
        return entry

    # Looking for this line:
    # Entry point: 0x400090
    for line in pwndbglib.info.files().splitlines():
    ...
    for name in ['_start', 'start', '__start', 'main']:
        try:
            return pwndbglib.symbol.address(name)
        except gdb.error:
            pass
...
def find_elf_magic(pointer, max_pages=1024, search_down=False, ret_addr_anyway=False):
    ...
    """
    addr = pwndbglib.memory.page_align(pointer)
    step = pwndbglib.memory.PAGE_SIZE
    if search_down:
        step = -step

    max_addr = pwndbglib.arch.ptrmask
    ...
    for i in range(max_pages):
        ...
        try:
            data = pwndbglib.memory.read(addr, 4)
...
def get_ehdr(pointer):
    ...
    # the ELF header.
    base = pwndbglib.memory.page_align(pointer)
...
def map_inner(ei_class, ehdr, objfile):
    ...
    for phdr in iter_phdrs(ehdr):
        ...
        memsz += pwndbglib.memory.page_offset(vaddr)
        memsz  = pwndbglib.memory.page_size_align(memsz)
        vaddr  = pwndbglib.memory.page_align(vaddr)
        offset = pwndbglib.memory.page_align(offset)

        # For each page described by this program header
        for page_addr in range(vaddr, vaddr+memsz, pwndbglib.memory.PAGE_SIZE):
            if page_addr in pages:
                ...
                page = pwndbglib.memory.Page(page_addr, pwndbglib.memory.PAGE_SIZE, flags, offset + (page_addr - vaddr))
                pages.append(page)
    ...
    for i in range(len(pages)-1):
        ...
        b_begin = b.vaddr
        if a_end != b_begin:
            gaps.append(pwndbglib.memory.Page(a_end, b_begin - a_end, 0, b.offset))

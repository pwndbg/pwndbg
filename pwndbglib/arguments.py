...
def get_syscall_name(instruction):
    ...
    try:
        abi     = pwndbglib.abi.ABI.syscall()
        syscall = getattr(pwndbglib.regs, abi.syscall_register)
        name    = pwndbglib.constants.syscall(syscall)
...
def get(instruction):
    ...
    if instruction.address != pwndbglib.regs.pc:
        return []

    try:
        abi = pwndbglib.abi.ABI.default()
    except KeyError:
        return []
    ...
    if CS_GRP_CALL in instruction.groups:
        ...
        name = pwndbglib.symbol.get(target)
        if not name:
            return []
    ...
    if name not in pwndbglib.functions.functions:
        name  = name.replace('_chk', '')
        name  = name.strip().lstrip('_')    # _malloc

    func = pwndbglib.functions.functions.get(name, None)
    ...
    if not func and target:
        typename = pwndbglib.ida.GetType(target)
        ...
        if typename:
            ...
            func = pwndbglib.funcparser.ExtractFuncDeclFromSource(typename + ';')

    if func:
        args = func.args
    else:
        args = [pwndbglib.functions.Argument('int', 0, argname(i, abi)) for i in range(n_args_default)]
...
def argname(n, abi=None):
    abi  = abi or pwndbglib.abi.ABI.default()
...
def argument(n, abi=None):
    ...
    if n < len(regs):
        return getattr(pwndbglib.regs, regs[n])

    n -= len(regs)

    sp = pwndbglib.regs.sp + (n * pwndbglib.arch.ptrsize)

    return int(pwndbglib.memory.poi(pwndbglib.typeinfo.ppvoid, sp))

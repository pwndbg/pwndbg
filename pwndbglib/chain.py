...
import pwndbglib.color.chain as C
...
LIMIT = pwndbglib.config.Parameter('dereference-limit', 5, 'max number of pointers to dereference in a chain')
...
def get(address, limit=LIMIT, offset=0, hard_stop=None, hard_end=0, include_start=True):
    ...
    for i in range(limit):
        ...
        try:
            ...
            if not pwndbglib.abi.linux and not pwndbglib.vmmap.find(address):
                break

            address = int(pwndbglib.memory.poi(pwndbglib.typeinfo.ppvoid, address))
...
def format(value, limit=LIMIT, code=True, offset=0, hard_stop=None, hard_end=0):
    ...
    rest = []
    for link in chain:
        symbol = pwndbglib.symbol.get(link) or None
        if symbol:
    ...
    if len(chain) == 1:
        enhanced = pwndbglib.enhance.enhance(chain[-1], code=code)
    ...
    # chain failed at that offset, so display that offset.
        ...
        enhanced = pwndbglib.enhance.enhance(chain[-2] + offset, code=code)

...
def byte(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.uchar, addr)
...
def ushort(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.ushort, addr)
...
def uint(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.uint, addr)
...
def pvoid(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.pvoid, addr)
...
def u8(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.uint8, addr)
...
def u16(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.uint16, addr)
...
def u32(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.uint32, addr)
...
def u64(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.uint64, addr)
...
def s8(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.int8, addr)
...
def s16(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.int16, addr)
...
def s32(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.int32, addr)
...
def s64(addr):
    ...
    """
    return readtype(pwndbglib.typeinfo.int64, addr)
...
def find_upper_boundary(addr, max_pages=1024):
    ...
    page is found.
    """
    addr = pwndbglib.memory.page_align(int(addr))
    try:
        for i in range(max_pages):
            pwndbglib.memory.read(addr, 1)
            # import sys
            # sys.stdout.write(hex(addr) + '\n')
            addr += pwndbglib.memory.PAGE_SIZE
            if addr > pwndbglib.arch.ptrmask:
                break
...
class Page(object):
    ...
    def __str__(self):
        width = 2 + 2 * pwndbglib.typeinfo.ptrsize
...
def update_min_addr():
    ...
    if pwndbglib.qemu.is_qemu_kernel():
        MMAP_MIN_ADDR = 0

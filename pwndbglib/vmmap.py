...
import pwndbglib.qemu
import pwndbglib.regs
import pwndbglib.remote
...
@pwndbglib.events.new_objfile
@pwndbglib.memoize.reset_on_stop
def get():
    if not pwndbglib.proc.alive:
        return tuple()
    pages = []
    pages.extend(proc_pid_maps())

    if not pages and pwndbglib.arch.current in ('i386', 'x86-64') and pwndbglib.qemu.is_qemu():
    ...
    if not pages:
        ...
        pages.extend(pwndbglib.stack.stacks.values())
...
def explore(address_maybe):
    ...
    address_maybe = pwndbglib.memory.page_align(address_maybe)

    flags = 4 if pwndbglib.memory.peek(address_maybe) else 0

    if not flags:
        return None

    flags |= 2 if pwndbglib.memory.poke(address_maybe) else 0
    flags |= 1 if not pwndbglib.stack.nx               else 0
...
@pwndbglib.events.stop
def explore_registers():
    for regname in pwndbglib.regs.common:
        find(pwndbglib.regs[regname])


@pwndbglib.events.exit
...
def proc_pid_maps():
    ...
    locations = [
        '/proc/%s/maps' % pwndbglib.proc.pid,
        '/proc/%s/map' % pwndbglib.proc.pid,
        '/usr/compat/linux/proc/%s/maps' % pwndbglib.proc.pid,
    ]

    for location in locations:
        try:
            data = pwndbglib.file.get(location)
            break
    ...
    for line in data.splitlines():
        ...
        page = pwndbglib.memory.Page(start, size, flags, offset, objfile)
        pages.append(page)
...
def monitor_info_mem():
    ...
    for line in lines:
        ...
        pages.append(pwndbglib.memory.Page(start, size, flags, 0, '<qemu>'))
...
def info_sharedlibrary():
    ...
    for line in gdb.execute('info sharedlibrary', to_string=True).splitlines():
        ...
        pages.extend(pwndbglib.elf.map(text, obj))
...
def info_files():
    ...
    for line in gdb.execute('info files', to_string=True).splitlines():
        ...
        pages.extend(pwndbglib.elf.map(vaddr, objfile))
...
def info_auxv(skip_exe=False):
    ...
    if not skip_exe and (entry or phdr):
        pages.extend(pwndbglib.elf.map(entry or phdr, exe_name))

    if base:
        pages.extend(pwndbglib.elf.map(base, '[linker]'))

    if vdso:
        pages.extend(pwndbglib.elf.map(vdso, '[vdso]'))
...
def find_boundaries(addr, name='', min=0):
    ...
    which are mapped.
    """
    start = pwndbglib.memory.find_lower_boundary(addr)
    end   = pwndbglib.memory.find_upper_boundary(addr)

    if start < min:
        start = min

    return pwndbglib.memory.Page(start, end - start, 4, 0, name)

aslr = False

@pwndbglib.events.new_objfile
@pwndbglib.memoize.while_running
def check_aslr():
    ...
    if pwndbglib.qemu.is_qemu_usermode():
        return vmmap.aslr

    # Systemwide ASLR is disabled
    try:
        data = pwndbglib.file.get('/proc/sys/kernel/randomize_va_space')
        if b'0' in data:
            vmmap.aslr = False
    ...
    if pwndbglib.proc.alive:
        try:
            data = pwndbglib.file.get('/proc/%i/personality' % pwndbglib.proc.pid)
...
@pwndbglib.events.cont
def mark_pc_as_executable():
    mapping = find(pwndbglib.regs.pc)

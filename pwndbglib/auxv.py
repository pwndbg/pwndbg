    ...
    def set(self, const, value):
        ...
        if name in ['AT_EXECFN', 'AT_PLATFORM']:
            try:
                ...
                value = value.cast(pwndbglib.typeinfo.pchar)
                value = value.string()
            except:
...
def use_info_auxv():
    lines = pwndbglib.info.auxv().splitlines()
...
def find_stack_boundary(addr):
    ...
    try:
        while True:
            if b'\x7fELF' == pwndbglib.memory.read(addr, 4):
                break
...
def walk_stack2(offset=0):
    ...
    p = gdb.Value(end).cast(pwndbglib.typeinfo.ulong.pointer())
    ...
    auxv = AUXV()
    while True:
        const = int((p+0).dereference()) & pwndbglib.arch.ptrmask
        value = int((p+1).dereference()) & pwndbglib.arch.ptrmask
...
def _get_execfn():
    ...
    if not pwndbglib.memory.peek(pwndbglib.regs.sp):
        return
    ...
    addr = pwndbglib.stack.find_upper_stack_boundary(pwndbglib.regs.sp)

    while pwndbglib.memory.byte(addr - 1) == 0:
        addr -= 1

    while pwndbglib.memory.byte(addr - 1) != 0:
        addr -= 1

    v = pwndbglib.strings.get(addr, 1024)
    if v:

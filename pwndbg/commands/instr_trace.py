...
parser.add_argument("-v","--verbose", default=False, action="store_true", help="Flag to dump backing file.")
...
async def _step_and_log(
    ...
    while(pwndbg.aglib.proc.alive()):
        ...
        if vmmap_object != None:
            ...
            if (page_): log_line = f"[{hex(current_pc)}]!{current_symbol} : {page_.objfile:<8}\n"
            else: log_line = f"[{hex(current_pc)}]!{current_symbol}\n"
...
def instr_trace(trace_file: str, end_address: int, verbose: bool) -> None:
    ...
    pwndbg.config.context_backtrace_lines.value = 0  # enable full backtrace
    ...
    try:
      print(message.info(f"instr-trace: Dumping instruction trace to {trace_file} until {hex(end_address)}..."))
      pwndbg.dbg.selected_inferior().dispatch_execution_controller(dump_trace_log)
      print(message.info(f"instr-trace: Complete."))
    except pwndbg.dbg_mod.Error as e:
      print(message.error(f"instr-trace Error: {e}"))
    pwndbg.config.context_backtrace_lines.value = old_val
    return

...
class GlibcMemoryAllocator(pwndbg.aglib.heap.heap.MemoryAllocator, Generic[TheType, TheValue]):
    ...
    # Largebin 31 (bin 95) isn't used on i386 when MALLOC_ALIGNMENT is 16, so
    # its value must be added manually.
    ...
    def malloc_alignment(self) -> int:
        ...
        if pwndbg.aglib.arch.name == "i386" and pwndbg.glibc.get_version() >= (2, 26):
            ...
            return 16
        # See
        # https://elixir.bootlin.com/glibc/glibc-2.37/source/sysdeps/generic/malloc-alignment.h#L27
    ...
    def check_chain_corrupted(self, chain_fd: List[int], chain_bk: List[int]) -> bool:
        ...
        if len(chain_fd) != len(chain_bk):
            ...
            # Forward and backward chains may have some overlap, we don't need to
            # recheck those chunks
            checked = set()
...
class HeuristicHeap(
            ...
            if data_section and data_section_address:
                ...
                for section_name in (".relr.dyn", ".rela.dyn", ".rel.dyn"):
                    ...
                    for relocation in relocations:
                        ...
                        # If addend is the offset of main_arena, then r_offset should be the
                        # offset of main_arena.next
                        if r_offset - next_field_offset == addend:
                            ...
                            # Note: Although RELA relocations have r_addend, some compiler will
                            # still put the addend in the location of r_offset, so we still need to
                            # check both cases
                            found = False
                            ...
                            if not found:
                                ...
                                found |= bytes(expected) == tmp
                            if found:
                                # This might be a false positive, but it is very unlikely, so should
                                # be fine :)
                ...
                # If we are still not able to find the main_arena, probably we are
                # debugging a binary with statically linked libc and no PIE enabled
                if not self._main_arena_addr:
                    # Try to find the default main_arena struct in the .data section
                    for i in range(0, size - self.malloc_state.sizeof, pwndbg.aglib.arch.ptrsize):
                        ...
                        if bytes(expected) == data_section_data[i : i + malloc_state_size]:
                            # This also might be a false positive, but it is very unlikely too, so
                            # should also be fine :)
    ...
    def has_tcache(self) -> bool:
        ...
        # There is no debug symbols, we determine the tcache_bins existence by
        # checking glibc version only
    ...
    def prompt_for_brute_force_thread_arena_permission(self) -> bool:
        ...
        print(
            ...
            + message.warn(
                "Note: This might take a while and might not be reliable, so if you can"
                " determine it by yourself or you have modified any of the arena, please"
                " do not use this."
            )
        )
    ...
    def prompt_for_brute_force_thread_cache_permission(self) -> bool:
        ...
        print(
            message.notice("We cannot determine the %s\n" % message.hint("tcache"))
            + message.notice(
                "Will you want to brute force it in the memory to determine the address"
                " instead of assuming it's at the beginning of the current thread's heap? (y/N)\n"
            )
            + message.warn(
                "Note: This might take a while and might not be reliable, so if you can"
                " determine it by yourself or your current arena is corrupted or you have"
                " modified the chunk for the tcache, please do not use this."
            )
        )
        return input().lower() == "y"
        ...
        if (
            ...
            if self.prompt_for_brute_force_thread_arena_permission():
                ...
                if found:
                    ...
                    print(
                        message.notice(
                            f"Found matching arena address {message.hint(hex(value))}"
                            f" at {message.hint(hex(address))}\n"
                        )
                    )
                    arena = Arena(value)
                ...
                print(
                    message.notice(
                        f"Cannot find {message.hint('thread_arena')},"
                        " the arena might be not allocated yet.\n"
                    )
                )
                return None
            raise SymbolUnresolvableError("thread_arena")
    ...
    def mallinfo(self) -> Type["pwndbg.aglib.heap.structs.CStruct2GDB"] | None:
        # TODO/FIXME: Currently, we don't need to create a new class for `struct
        # mallinfo` because we never use it.
    ...
    def get_sbrk_heap_region(self) -> pwndbg.lib.memory.Page:
        ...
        if not self._mp_addr:
            try:
                ...
                # Should only raise SymbolUnresolvableError, but the heuristic heap
                # implementation is still buggy so catch all exceptions for now.
                pass

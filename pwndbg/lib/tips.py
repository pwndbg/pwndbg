...
# GDB specific tips
GDB_TIPS: List[str] = [
    "GDB's `apropos <topic>` command displays all registered commands that are related"
    " to the given <topic>",
    ...
    "ddr, data)`, `pwndbg.aglib.vmmap.get()` and so on!",
    "GDB's `set directories <path>` parameter can be used to debug e.g. glibc sources "
    "like the malloc/free functions!",
    ...
    ": `| <cmd> | <prog>`",
    "Prefixing a command with `!` in GDB will execute it as a shell command, e.g.: `!l"
    "s` or `!cat flag.txt`",
]
...
PWNDBG_TIPS: List[str] = [
    ...
    "or info mem` QEMU gdbstub command (use `set kernel-vmmap-via-page-tables off` for"
    " that)",
    "Use the `vmmap` command for a better & colored memory maps display (than the GDB'"
    "s `info proc mappings`)",
    "Use the `telescope` command to dereference a given address/pointer multiple times"
    " (if the dereferenced value is a valid ptr; see `config telescope` to configure i"
    "ts behavior)",
    "Use the `context` (or `ctx`) command to display the context once again. You can r"
    "econfigure the context layout with `set context-section <sections>` or forward th"
    "e output to a file/tty via `set context-output <file>`. See also `config context`"
    " to configure it further!",
    "Disable Pwndbg context information display with `set context-sections ''`",
    "Pwndbg context displays where the program branches to thanks to emulating few ins"
    "tructions into the future. You can disable this with `set emulate off` which may "
    "also speed up debugging",
    "Use the `canary` command to see all stack canary/cookie values on the stack (base"
    "d on the *usual* stack canary value initialized by glibc)",
    "Use the `procinfo` command for better process introspection (than the GDB's `info"
    " proc` command)",
    "Want to display each context panel in a separate tmux window? See https://github."
    "com/pwndbg/pwndbg/blob/dev/FEATURES.md#splitting--layouting-context",
    'Use `$base("heap")` to get the start address of a [heap] memory page',
    "Use the `errno` (or `errno <number>`) command to see the name of the last or prov"
    "ided (libc) error",
    ...
    "`break-if-taken` and `break-if-not-taken` commands sets breakpoints after a given"
    " jump instruction was taken or not",
    "`stepuntilasm <assembly-instruction [operands]>` steps program forward until matc"
    "hing instruction occures",
    "Use `plist` command to dump elements of linked list",
    "If your program has multiple threads they will be displayed in the context displa"
    "y or using the `context threads` command",
    "Use `track-got enable|info|query` to track GOT accesses - useful for hijacking co"
    "ntrol flow via writable GOT/PLT",
    "Need to `mmap` or `mprotect` memory in the debugee? Use commands with the same na"
    "me to inject and run such syscalls",
    "Use `hi` to see if a an address belongs to a glibc heap chunk",
    "Use `contextprev` and `contextnext` to display a previous context output again wi"
    "thout scrolling",
    "Try splitting the context output into multiple TUI windows using `layout pwndbg` "
    "(`tui disable` or `ctrl-x + a` to go back to CLI mode)",
]
...
LLDB_TIPS: List[str] = [
    ...
    "The `breakpoint set --func-regex <regex>` command sets breakpoints on functions matching"
    " a regular expression",
    "Use `target modules list` to see all loaded modules in your process",
    "LLDB's `image lookup` command helps find symbols, addresses, and files in the executable"

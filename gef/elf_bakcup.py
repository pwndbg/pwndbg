...
def find_sections(pointer):
    ...
    for i in range(0, phnum):
        ...
        for page in (vaddr, vaddr+memsz, gef.memory.PAGE_SIZE):
            section = MemoryPage(vaddr, gef.memory.PAGE_SIZE, flags, offset + (page-vaddr))
    ...
    last = sections[0]
    for section in list(sections[1:]):
        if last.perm == section.perm and last.vaddr+last.memsz == section.vaddr:
            last.memsz += section.memsz

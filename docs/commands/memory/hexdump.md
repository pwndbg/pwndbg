<!-- THIS PART OF THIS FILE IS AUTOGENERATED. DO NOT MODIFY IT. See scripts/generate-docs.sh -->
# hexdump

```text
usage: hexdump [-h] [-C [{py,c}]] [-H HIGHLIGHT] [address] [count]

```

Hexdumps data at the specified address or module name.
### Positional arguments

|Positional Argument|Help|
| :--- | :--- |
|address|Address or module name to dump (default: $sp)|
|count|Number of bytes to dump (default: 64)|

### Optional arguments

|Short|Long|Help|
| :--- | :--- | :--- |
|-h|--help|show this help message and exit|
|-C|--code|Output as Python or C code data definition (default: py)|
|-H|--highlight|Comma-separated hex byte patterns to highlight, e.g. '00,0a,4?'. '?' matches any nibble. Useful for spotting bad chars in a payload|

<!-- END OF AUTOGENERATED PART. Do not modify this line or the line below, they mark the end of the auto-generated part of the file. If you want to extend the documentation in a way which cannot easily be done by adding to the command help description, write below the following line. -->
<!-- ------------\>8---- ----\>8---- ----\>8------------ -->

...
||--refresh|Drop the cached memory map (used by default on macOS) and re-fetch it. See the `vmmap-cache` config option.|
||--json|Output memory map in JSON format for programmatic consumption|

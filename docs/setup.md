...
```{.bash .copy}
dnf install ./pwndbg-2026.07.27.x86_64.rpm
```
DEB-based Systems (Debian/Ubuntu/Kali):
```{.bash .copy}
apt install ./pwndbg_2026.07.27_amd64.deb
```
Alpine:
```{.bash .copy}
apk add --allow-untrusted ./pwndbg_2026.07.27_x86_64.apk
```
Arch Linux:
```{.bash .copy}
pacman -U ./pwndbg-2026.07.27-1-x86_64.pkg.tar.zst
```
### Distro packages

...
When installing with GDB, you may also download a package file to install through your package manager of choice. Download the package from the [releases page](https://github.com/pwndbg/pwndbg/releases) and pick the appropriate download from the second table.
...
You can manually download a portable release on the [Pwndbg releases page](https://github.com/pwndbg/pwndbg/releases). There are separate releases for GDB and LLDB. Use the first table to pick the appropriate download for your system architecture. You can then unpack the archive with:
```{.bash .copy}
...
??? note "Removing Quarantine Flags (macOS)"
...
This will get you the same setup as in [System GDB, sourced from `~/.gdbinit`](#system-gdb-sourced-from-gdbinit). You can update with `git pull`.

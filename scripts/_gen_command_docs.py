...
autogen_end_marker1 = (
    "<!-- END OF AUTOGENERATED PART. Do not modify this line or the line below,"
    " they mark the end of the auto-generated part of the file. If you want to "
    "extend the documentation in a way which cannot easily be done by adding to"
    " the command help description, write below the following line. -->\n"
)
    ...
    for obj_name in dir(pwndbg.commands):
        ...
        for fn_name in dir(mod):
            ...
            if category is None:
                ...
                print(
                    f"ERROR: Command function {fn_name} in {obj_name} does "
                    "not have an assigned category."
                )
                sys.exit(4)
...
def convert_to_markdown(filename: str, command: CommandObj) -> str:
    ...
    autogen_warning = (
        "<!-- THIS PART OF THIS FILE IS AUTOGENERATED. DO NOT MODIFY IT."
        " See scripts/generate_docs.sh -->"
    )
...
def verify_files(filename_to_markdown: Dict[str, str]) -> str | None:
    ...
    for filename, markdown in filename_to_markdown.items():
        ...
        if not (
            ...
            return (
                f'Expected autogenerated end markers in {filename} @ '
                f'lines {mkdlen} and {mkdlen + 1}. Instead found '
                f'"{file_data[mkdlen]}" and "{file_data[mkdlen + 1]}".'
            )

        for i in range(mkdlen):
            if file_data[i] != markdown[i]:
                return (
                    f'File {filename} differs from autogenerated on line {i}.\n'
...
def update_files(filename_to_markdown: Dict[str, str]):
    ...
    for filename, markdown in filename_to_markdown.items():
        ...
        with open(filename, "r+") as file:
            ...
            for i in reversed(range(len(file_data))):
                if file_data[i] == autogen_end_marker2:
                    if i == 0 or file_data[i - 1] != autogen_end_marker1:
                        print(
                            f"ERROR: In file {filename} found the second autogen marker,"
                            f" but couldn't find the first ({autogen_end_marker1})."
                        )
                        sys.exit(6)
                    marker_idx = i - 1
                    break
            ...
            if marker_idx == -1:
                print(
                    f"ERROR: In file {filename} couldn't find autogen marker "
                    f"({autogen_end_marker2})."
                )
                sys.exit(7)

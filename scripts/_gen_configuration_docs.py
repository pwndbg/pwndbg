...
def extract_params() -> Dict[str, list[Parameter]]:
    ...
    assert len(scope_dict) == len(pwndbg.lib.config.Scope) and ("The amount of detected scopes "
...
def convert_to_markdown(scoped: Dict[str, list[Parameter]]) -> Dict[str, str]:
    ...
    for scope, param_list in scoped.items():
        ...
        for param in param_list:
            ...
            help_docstring = param.help_docstring.replace(HELP_DEFAULT_PREFIX, f"**{HELP_DEFAULT_PREFIX}**")
            help_docstring = help_docstring.replace(HELP_VALID_VALUES_PREFIX, f"**{HELP_VALID_VALUES_PREFIX}**")
...
def check_index(scoped_params: Dict[str, list[Parameter]]):
    assert(len(scoped_params.keys()) == 3 and "It seems a new scope has been added, "

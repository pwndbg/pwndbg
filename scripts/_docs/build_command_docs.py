...
def get_markdown_body(cmd: ExtractedCommand, level: int = 0) -> str:
    """
    level is 0 for commands, 1 for subcommands, 2 for subsubcommands...
    """
    mdFile = MdUtils(cmd.filename)

    leveled_header: str = "#" * (level + 3)
    ...
    if cmd.subcommands:
        ...
        for subcmd in cmd.subcommands:
            print("subcmd! ", subcmd)
            subcmd_md: str = get_markdown_body(subcmd, level + 1)
            mdFile.write(f"### **{cmd.name} {subcmd.name}**")
            if subcmd.aliases:
                aliases = ", ".join(subcmd.aliases)

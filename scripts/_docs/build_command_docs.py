...
def get_markdown_body(
    cmd: ExtractedCommand, is_top_level: bool = True, built_subcmd_preface: str = ""
) -> str:
    """
    is_top_level=True means its not a subcommand.

    For `decompiler-integration install ida`, building the documentation for the `ida` subcommand, we will have
    `built_subcmd_preface == "decompiler-integration install"` and `cmd.name == "ida"`.
    """
    ...
    if cmd.positionals:
        ...
        mdFile.write(f"{leveled_header} Positional arguments\n")
        mdFile.new_table(
            columns=2,
    ...
    if cmd.optionals:
        ...
        mdFile.write(f"{leveled_header} Optional arguments\n")
        mdFile.new_table(
            columns=3,
    ...
    if cmd.subcommands:
        built_subcmd_preface = built_subcmd_preface + f"{cmd.name} "
        # Add all the subcommand's bodies as well.
        for subcmd in cmd.subcommands:
            # Title is ###, and get_markdown_body will make arguments be ####.
            mdFile.write(f"### **{built_subcmd_preface}{subcmd.name}**")
            # This also generates the bodies and titles of all subcommands of this
            # subcommand.
            subcmd_md: str = get_markdown_body(subcmd, False, built_subcmd_preface)
            # We don't actually need to explicitly write out aliases because they
            # will be shown in the command help.
            # Also when I tried to show aliases as (alias) I had (c) turn into ©
            mdFile.write("\n" + subcmd_md + "\n")

    if cmd.examples:
        # Put the examples into a code block so they are formatted sensically.
        mdFile.write(f"{leveled_header} Examples\n```text\n" + cmd.examples + "\n```\n")

    if cmd.notes:
        # The author of the note should make sure it make sense in markdown.
        mdFile.write(f"{leveled_header} Notes\n" + cmd.notes + "\n")

    if cmd.pure_epilog:
        mdFile.write(f"{leveled_header} Extra\n" + cmd.pure_epilog + "\n")

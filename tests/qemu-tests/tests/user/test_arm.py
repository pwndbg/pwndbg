...
def test_arm_simple_branch(qemu_assembly_run):
    """
    Simple test to ensure branches are being followed correctly and that they
    are remembered when stepping past them
    """
...
def test_arm_interworking_branch(qemu_assembly_run):
    ...
    This requires Capstone to be synced with Unicorn, and for Unicorn to properly
    execute Thumb instructions.

    The code starts in Arm mode, then transitions to Thumb mode.
    If this breaks, it is likely that something has stopped Unicorn from correctly
    running Thumb mode instructions.

    Additionally, the lowest bit of the target must always be 0 - although
    interworking branches that transition to Thumb mode appear to write a
    1 to the lowest bit, in the hardware the bit is directed to the Thumb
    bit in the CPSR flags register.
...
def test_arm_implicit_branch(qemu_assembly_run):
    ...
    This test contains a "add" instruction that causes the PC to change. We want there to be
    a <target> displayed, and a space after it in the disasm
    """
...
def test_arm_stack_pointer_check(qemu_assembly_run):
    ...
    We are testing to ensure that Unicorn does not crash on this access to the stack pointer
    (the pop instruction). If the emulator registers are not instantiated in the correct
    order (CPSR is written AFTER stack pointer), the stack pointer will be reset to zero due
    to banked registers: https://github.com/unicorn-engine/unicorn/issues/1984

    If this test fails (annotations after the str/pop don't show), it likely means that the
    stack pointer has the incorrect value.
...
def test_arm_exclusive_store(qemu_assembly_run):
    ...
    expected = (
        ...
        "   0x200ec <_start+24>    add    r0, r0, r1          "
        "R0 => 0x8768443e (0x3011d + 0x87654321)\n"
...
def test_arm_logical_shifts(qemu_assembly_run):
    """
    Shifts have a different underlying Capstone representation if it's
    an constant or a register offset. This test ensures we handle both cases.
    """
...
def test_arm_negative_disponent(qemu_assembly_run):
    """
    Negative disponents are now represented by a positive offset and
    a flag that indicates it should be subtracted.
...
def test_arm_negative_index_register(qemu_assembly_run):
    ...
    This has a specific encoding that has changed in Capstone in the past,
    so we test to make sure we are handling it correctly.
    """
    ...
    expected = (
        ...
        "   0x200d8 <_start+4>     add    r1, r1, #4                "
        "R1 => 0x30114 (msg+4) (0x30110 + 0x4)\n"
        "   0x200dc <_start+8>     add    r2, r1, #4                "
        "R2 => 0x30118 (msg+8) (0x30114 + 0x4)\n"
        "   0x200e0 <_start+12>    mov    r3, #4                    R3 => 4\n"
        "   0x200e4 <_start+16>    mov    r4, #2                    R4 => 2\n"
        "   0x200e8 <_start+20>    ldr    r5, [r1, -r3]             "
        "R5, [msg] => 0x44434241 ('ABCD')\n"
        "   0x200ec <_start+24>    ldr    r6, [r2, -r4, lsl #2]     "
        "R6, [msg] => 0x44434241 ('ABCD')\n"
        "   0x200f0 <_start+28>    nop    \n"
        "   0x200f4 <_start+32>    nop    \n"
        "   0x200f8 <_start+36>    nop    \n"
        "   0x200fc <_start+40>    nop    \n"
...
def test_arm_it_block(qemu_assembly_run):
    """
    The Unicorn engine cannot be paused in the IT block, so we need to
    handle these instructions specially.

    Additionally, if we are halfway through an IT block, and then copy
    the the process state into the emulator, it will finish the
...
def test_arm_it_block_cached_thumb_mode(qemu_assembly_run):
    """
    This test ensures that we handle transitions to Thumb mode
    correctly once the emulator has been disabled.

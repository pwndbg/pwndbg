...
async def test_command_break_if_x64(ctrl: Controller, binary: Path) -> None:
    """
...
async def continue_and_test_pc(ctrl: Controller, stop_label: str) -> None:
    ...
    maybe_address = pwndbg.dbg.selected_inferior().lookup_symbol(stop_label)
    assert maybe_address is not None
    assert pwndbg.aglib.regs.pc == int(maybe_address)

...
@pwndbg_test
async def test_process_launch_stdio(ctrl: Controller, tmp_path: Path) -> None:
    input_file = tmp_path / "stdin.txt"
    output_file = tmp_path / "stdout.txt"
    error_file = tmp_path / "stderr.txt"
    input_file.write_text("1337\n", encoding="utf-8")

    await ctrl.launch(
        get_binary("stdin.native.out"),
        stdin=input_file,
        stdout=output_file,
        stderr=error_file,
    )
    ...
    assert output_file.read_text(encoding="utf-8") == "stdout: 1337\n"
    assert error_file.read_text(encoding="utf-8") == "stderr: 1337\n"

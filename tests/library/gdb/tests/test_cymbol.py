...
def test_cymbol(start_binary):
    ...
    def dummyf(custom_structure_name: str, custom_structure_path: str = "") -> None:
        return

    assert pwndbg.commands.cymbol.OnlyWhenStructFileExists(dummyf)("dummy") is None
    assert pwndbg.commands.cymbol.OnlyWhenStructFileExists(dummyf)("example") is True
